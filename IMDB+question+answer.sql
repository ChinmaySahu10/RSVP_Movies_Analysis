...
SELECT DISTINCT(g.genre), AVG(m.duration)
FROM genre g
JOIN movie m
ON g.movie_id = m.id
GROUP BY g.genre;
...
JOIN ratings r
ON m.id = r.movie_id
WHERE r.avg_rating > 8  AND production_company IS NOT NULL -- have to put this conition or else NULL values tops the list
...
WHERE r.total_votes > 1000 AND m.country = 'USA' AND YEAR(date_published) = 2017 AND MONTH(date_published) = 03
GROUP BY g.genre
...
Select 
  sum(case when id is null then 1 else 0 end) id,
  sum(case when name is null then 1 else 0 end) name,
  sum(case when height is null then 1 else 0 end) height,
  sum(case when date_of_birth is null then 1 else 0 end) date_of_birth,
  sum(case when known_for_movies is null then 1 else 0 end) known_for_movies
from names;
...
SELECT
   ...
   DENSE_RANK() OVER( ORDER BY ROUND(SUM(r.avg_rating*total_votes) / SUM(total_votes), 2) DESC) AS actor_rank 
FROM names n 
...
AND CHAR_LENGTH(languages) - CHAR_LENGTH(REPLACE(languages, ',', '')) >= 1 -- considering only movies with more than one language
GROUP BY production_company
LIMIT 2;
...
AND r.avg_rating > 8 
AND g.genre = 'drama' 
GROUP BY actress_name, r.avg_rating
LIMIT 3;
